...
def get_connection():
    return snowflake.connector.connect(
        ...
        password  = st.secrets.get("SNOWFLAKE_PASSWORD", "Sinchanagowda@123"),
...
def load_all_data():
    ...
    return (df_sales, df_inventory, df_inventory_detail,
            ...
            df_transactions, df_competitor, df_economic, df_weather)

# ── Custom CSS ─────────────────────────────────
st.markdown("""
<style>
    .main { background-color: #0F172A; }
    .stMetric { background-color: #1E293B; border-radius: 10px; padding: 10px; }
...
with st.spinner("Loading data from Snowflake..."):
    (df_sales, df_inventory, df_inventory_detail,
     df_suppliers, df_customers, df_fraud,
     df_transactions, df_competitor, df_economic,
     df_weather) = load_all_data()
...
if page == "📊 Executive Dashboard":
    ...
    c4.metric("✅ Healthy Stock",  f"{healthy_stock}")
    c5.metric("⚠️ Fraud Alerts",  f"{fraud_count}")
    c6.metric("🏭 Top Supplier",   top_supplier[:15])
    ...
    with col1:
        if len(df_sales) > 0:
            fig = px.line(df_sales, x='TRANSACTION_DATE', y='REVENUE',
    ...
    with col2:
        if len(df_inventory) > 0:
            fig = px.pie(df_inventory, names='STOCK_STATUS', values='COUNT',
                        title='Inventory Health',
                        color_discrete_map={'HEALTHY': '#16A34A',
                                            'LOW_STOCK': '#D97706',
                                            'REORDER_NOW': '#DC2626',
                                            'OUT_OF_STOCK': '#7F1D1D'},
                        **CHART_STYLE)
    ...
    with col3:
        if len(df_suppliers) > 0:
            fig = px.bar(df_suppliers, x='SUPPLIER_NAME', y='RELIABILITY_SCORE',
                        color='RELIABILITY_TIER', title='Supplier Reliability',
                        **CHART_STYLE)
            st.plotly_chart(fig, use_container_width=True)
    with col4:
        if len(df_transactions) > 0:
            fig = px.pie(df_transactions, names='PAYMENT_METHOD', values='COUNT',
                        title='Payment Methods', **CHART_STYLE)
            st.plotly_chart(fig, use_container_width=True)
...
# ══════════════════════════════════════════════
    ...
    st.caption("For Sales Manager — Revenue, units, and trends")

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("💰 Total Revenue",  f"${total_revenue:,.0f}")
    c2.metric("📦 Total Units",    f"{total_units:,.0f}")
    c3.metric("📈 Avg Rev/Day",    f"${total_revenue/180:,.0f}")
    c4.metric("💳 Payment Types",  "5 Methods")
    ...
    with col1:
        if len(df_sales) > 0:
            fig = px.area(df_sales, x='TRANSACTION_DATE', y='UNITS',
                         color='STORE_ID', title='Daily Units Sold',
                         **CHART_STYLE)
            st.plotly_chart(fig, use_container_width=True)
    with col2:
        if len(df_transactions) > 0:
            fig = px.pie(df_transactions, names='PAYMENT_METHOD',
                        values='COUNT', title='Payment Methods',
                        **CHART_STYLE)
            st.plotly_chart(fig, use_container_width=True)
...
# ══════════════════════════════════════════════
    ...
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("✅ Healthy Items",  f"{healthy_stock}")
    c2.metric("⚠️ Reorder Now",   f"{reorder_count}")
    c3.metric("📉 Low Stock",      f"{int(df_inventory[df_inventory['STOCK_STATUS']=='LOW_STOCK']['COUNT'].sum()) if len(df_inventory)>0 else 0}")
    ...
    with col2:
        if len(df_inventory_detail) > 0:
            fig = px.bar(df_inventory_detail.head(20),
                        x='PRODUCT_ID', y='CURRENT_STOCK_LEVEL',
                        color='STOCK_STATUS', title='Stock Levels by Product',
                        color_discrete_map={'HEALTHY': '#16A34A',
                                            'LOW_STOCK': '#D97706',
                                            'REORDER_NOW': '#DC2626',
                                            'OUT_OF_STOCK': '#7F1D1D'},
...
# ══════════════════════════════════════════════
    ...
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("🛍️ Total SKUs",    "200 Products")
    c2.metric("🏪 Total Stores",  "50 Stores")
    ...
    with col1:
        if len(df_forecast) > 0:
            fig = px.bar(df_forecast, x='PRODUCT_ID', y='AVG_DEMAND',
                        title='Average Demand by Product', **CHART_STYLE)
            st.plotly_chart(fig, use_container_width=True)
    with col2:
        if len(df_spikes) > 0:
            fig = px.bar(df_spikes, x='PRODUCT_ID', y='SPIKE_COUNT',
                        color='SPIKE_COUNT', title='Demand Spikes by Product',
                        **CHART_STYLE)
            st.plotly_chart(fig, use_container_width=True)
    ...
    with col4:
        if len(df_economic) > 0:
            fig = px.bar(df_economic, x='CATEGORY', y='AVG_ELASTICITY',
                        color='AVG_LIFT', title='Price Elasticity by Category',
                        **CHART_STYLE)
            st.plotly_chart(fig, use_container_width=True)
...
# ══════════════════════════════════════════════
    ...
    with col1:
        if len(df_inventory) > 0:
            fig = px.pie(df_inventory, names='STOCK_STATUS', values='COUNT',
                        title='Stock Health', **CHART_STYLE)
            st.plotly_chart(fig, use_container_width=True)
    with col2:
        if len(df_inventory_detail) > 0:
            fig = px.bar(df_inventory_detail.head(20),
                        x='PRODUCT_ID', y='CURRENT_STOCK_LEVEL',
                        color='STOCK_STATUS', title='Stock by Product',
                        **CHART_STYLE)
            st.plotly_chart(fig, use_container_width=True)
    ...
    with col3:
        if len(df_weather) > 0:
            fig = px.line(df_weather, x='OBSERVATION_DATE', y='AVG_TEMP',
                         color='WEATHER_CONDITION_CODE',
                         title='Weather Trends', **CHART_STYLE)
            st.plotly_chart(fig, use_container_width=True)
    with col4:
        if len(df_suppliers) > 0:
            fig = px.scatter(df_suppliers,
...
# ══════════════════════════════════════════════
    ...
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("🏭 Total Suppliers", "30")
    c2.metric("⭐ Top Supplier",    top_supplier[:15])
    c3.metric("✅ HIGH Tier",       f"{high_tier}")
    c4.metric("📦 Total Batches",   f"{total_batches:,}")
...
# ══════════════════════════════════════════════
    ...
    c2.metric("⭐ Avg Loyalty Pts", f"{avg_loyalty:,.0f}")
    ...
    with col1:
        if len(df_customers) > 0:
            fig = px.bar(df_customers, x='LOYALTY_TIER', y='COUNT',
                        color='LOYALTY_TIER', title='Customer Segments',
                        color_discrete_map={'GOLD': '#F59E0B',
                                            'SILVER': '#94A3B8',
                                            'BRONZE': '#B45309'},
                        **CHART_STYLE)
            st.plotly_chart(fig, use_container_width=True)
    with col2:
        if len(df_customers) > 0:
            fig = px.bar(df_customers, x='LOYALTY_TIER', y='AVG_POINTS',
                        color='LOYALTY_TIER', title='Avg Loyalty Points',
                        color_discrete_map={'GOLD': '#F59E0B',
                                            'SILVER': '#94A3B8',
                                            'BRONZE': '#B45309'},
                        **CHART_STYLE)
    ...
    with col3:
        if len(df_customers) > 0:
            fig = px.bar(df_customers, x='LOYALTY_TIER', y='AVG_SESSION',
                        color='LOYALTY_TIER', title='Avg App Session Duration',
                        **CHART_STYLE)
            st.plotly_chart(fig, use_container_width=True)
    with col4:
        ...
        campaigns = [
            ...
            ("🌟 ALL",    "Holiday Special",       "1,000 customers","Email+Push+Social", "#16A34A"),
        ]
...
# ══════════════════════════════════════════════
    ...
    with col2:
        ...
        alerts = [
            (f"🚨 {out_of_stock} Products OUT OF STOCK", "Immediate reorder required — Auto POs created", "#DC2626"),
            (f"⚠️ {fraud_count} High-Risk Fraud Transactions", "Review flagged transactions immediately", "#D97706"),
            ("⚠️ 1,687 Critical Supply Chain Risks", "Activate backup suppliers immediately", "#D97706"),
            ("✅ All 20 AI Agents Running", "System operating normally", "#16A34A"),
            ("ℹ️ Digital Twin: Festival = $9,904 revenue", "Stock up for festival season", "#2563EB"),
        ]
